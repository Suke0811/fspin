...
def test_exception_logging_and_warning(caplog, capsys):
    ...
    # Log should contain the traceback
    assert any("Traceback" in r.exc_text or (r.exc_info and "Traceback" in str(r.exc_info)) for r in caplog.records if r.exc_info or r.exc_text), caplog.text
...
def test_reporting_setup_terminal_logging():
    """Test _setup_terminal_logging (lines 7-24 in reporting.py)"""
    with patch('logging.getLogger') as mock_get_logger:
        mock_root = MagicMock()
        mock_root.handlers = []  # No root handlers
        mock_get_logger.return_value = mock_root
        
        # Mocking reporting.logger
        with patch('fspin.reporting.logger') as mock_logger:
            mock_logger.handlers = [logging.NullHandler()]
